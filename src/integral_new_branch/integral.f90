...
!---------------------------------------------------------------------------
      ...
      subroutine integral3d(atom, na, ns, nr, int_res)
      
      use parameter_def
      ! argument
      integer, intent(in) :: atom, na, ns, nr
      ...
      do ka = 1, na
         ...
         do js = 1, ns
            ...
            do ir = 1, nr
               ...
               tmp_coord(2) = 0.0d0
               tmp_coord(3) = 0.0d0
               tmp_coord(1) = rcoords(ir) * scoords(1, is) 
               tmp_coord(2) = rcoords(ir) * scoords(2, is)
               tmp_coord(3) = rcoords(ir) * scoords(3, is)
               ...
               int_rval(js) = int_rval(js) + rwgts(ir) * ( pwgts(ka, (ir - 1) * ns + is) * charx * sine) * rcoord(ir) **2 
            end do
